...
| `hand`(손글씨) | Nanum Pen Script(`--font-hand`) | 400 | 28~72(맥락별) | 손글씨 제목 12곳(책장/발제/모임 기록/내 책장/다음 모임/모바일 탭) — PRD §4-2 OQ3 확정 |
...
### 3-1. 브랜드/포인트 (기존 유지)
- `point` `#E8412F` — 강조·선택 링·좋아요·태그라인·검색 캐럿. **버튼 주색 아님**.
...
- **`point` = `#E8412F`** — **강조/포인트**(선택 링·좋아요·태그라인·검색 캐럿). 버튼 주색 아님.

→ `styling-design-system`의 `--primary=#E8412F` → **`#000000`로 재매핑**, 오렌지는 `point`(기존 brand 토큰) 유지.
...
- **OQ2 primary** → ✅ **primary=검정(#000)**, point=오렌지(#E8412F) 분리(§3-2).
