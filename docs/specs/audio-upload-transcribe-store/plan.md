...
- **RLS(v1 최소):** RLS 활성화. 인증 사용자는 **자기 row만 SELECT** 가능
  ...
  세부 권한 정책은 추후 반복.
- **대용량 처리:** `stt-submit`이 R2 → RTZR로 **스트리밍 전달**. UI는 압축 포맷
  (m4a/mp3) 권장 + 파일 크기 상한(기본 **500MB**)으로 대형 WAV를 막는다.
...
| 리스크 | 영향 | 완화 |
|--------|------|------|
| 대형 WAV가 Edge Function 메모리/시간 한계 초과 | 높음 | 스트리밍 전달 + UI 포맷 권장/크기 상한(500MB); T5에서 실측 |
