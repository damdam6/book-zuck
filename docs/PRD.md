...
> 버전: v1.0 (2026-09-07) · 상태: **초안 — 검토 대기**
...
| 역할 | 프로토타입 | 구현 토큰 | 비고 |
|---|---|---|---|
| 포인트(활성 탭·번호·별·"다음 모임"·체크박스) | `#E8412F` | `orange-500` `#F83C00` (point) | ⚠️ OQ4 — 하나로 통일 |
...
| M | 범위 | 스펙 폴더 | 완료 정의 |
|---|---|---|---|
| **M0** | 브랜드·토큰 보강 + 공통 셸 | `app-shell` | 로고 SVG, `--font-hand`, 참여자 9색, 브레이크포인트 토큰, 헤더·탭바, `ProfileModal→/my` |
...
| OQ4 | 포인트 색 `#E8412F`(프로토) vs `#F83C00`(토큰) | 전 페이지 | 토큰 `#F83C00`로 통일 |
